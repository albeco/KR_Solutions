...
#define DEFAULTNTAB 8
#define MAXTABNO 100 // maximum length of tabs list
#define ENDOFTABS 0

/* Accepts command line arguments -m and -n to start tabs at column m
   and repeating every n columns. */
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 1) // default tabs
      tablist[0] = 0;
    else if (argc > 1) {
      if (*argv[1] == '-') { // expecting command like entab -m -n  
	tablist[0] = atoi(argv[1] + 1);
	tablist[1] = 0;
	if (argc == 3) {
	  if (*argv[2] == '-')
	    ntab = atoi(argv[2] + 1);
	  else {
	    printf("%s error: second dashed argument expected after a dashed one\n",
		   argv[0]);
	    return -1;
	  }
	}
	else if (argc > 3) {
	  printf("%s error: max 2 dash arguments allowed\n", argv[0]);
	  return -1;
	}
      }
      else { // expecting list of tabs
	int i = 0;
