...
/* expr: RPN calculator from command line, based on 'kr_ex4-6.c'. Note
that some operators, like '*', '//', '<', '>' do not currently work
from command line and should be renamed.  Alredy changed are '.'
instad of '*' for multiplication and ':' instead of '//' for
division */
...
int main(int argc, char *argv[])
{
  ...
  while (--argc > 0) {
    ...
    case MATHFUN:
      ...
      break;
    case USERVARIABLE:
      push(NAN, **argv); // first argument ignored and replaced with current variable value
      break;
    default:
